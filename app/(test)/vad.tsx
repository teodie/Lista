...
import { initWhisperVad, libVersion } from 'whisper.rn' // whisper.rn
import type { WhisperVadContext, VadSegment } from 'whisper.rn'
import Button from '@/app/(test)/Button'
import { createDir, fileDir, vadModelHost, toTimestamp } from './util'
import { WavFileWriter } from 'whisper.rn/utils/WavFileWriter.js'
import LiveAudioStream from '@fugood/react-native-audio-pcm-stream'

const sampleFile = require('@/assets/recordings/samples_jfk.wav')
...
export default function VadExample() {
  ...
  const player = useAudioPlayer(recordFile);
          ...
          <Button
            ...
            onPress={async () => {
              ...
              if (segments.length === 0) {
                ...
                const resultText = `Recorded Data - Detected ${segments.length
                  } speech segments:
          ...
          <Button
            ...
            onPress={async () => {
              ...
              if (segments.length === 0) {
                ...
                const resultText = `Sensitive Detection - Found ${segments.length
                  } speech segments:
Detection time: ${endTime - startTime}ms in ${mode} mode
...
${segments
        ...
        <Button
          ...
          onPress={async () => {
            ...
            log(`Recording filepath: ${recordFile} exist`)

            const recorded = useAudioPlayer(recordFile)
